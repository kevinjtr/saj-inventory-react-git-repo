...
export default function Eng4900(props) {
  ...
  const FormTabs = {0:'my_forms', 1:'sign_forms'}
  ...
  const [searchFields_, setSearchFields_] = React.useState({
    0:{
  ...
  const handleTableDelete = async (rowData) => {
    ...
    if(event.target.value == ''){
      setSearchFields_({...searchFields_,  [tabs] : {...searchFields_[tabs], [event.target.name] : {...searchFields_[tabs][event.target.name], value: event.target.value, options: OPTIONS_DEFAULT}} })
		}else{
			setSearchFields_({...searchFields_,  [tabs] : {...searchFields_[tabs], [event.target.name] : {...searchFields_[tabs][event.target.name], value: event.target.value}} })
    }
    ...
    setSearchFields_({...searchFields_,  [tabs] : {...searchFields_[tabs], [event.target.name] : {...searchFields_[tabs][event.target.name], options : opts}} })
	}
    ...
    setSearchFields_({...searchFields_,  [tabs] : {...searchFields_[tabs], [event.target.name] : {...searchFields_[tabs][event.target.name], blanks : blks}} })
  }
  ...
  const handleSearch = async (e=null,onLoad=false) => {
    ...
    Object.keys(searchFields_[tabs]).map(key => {
      fields_obj[key] = onLoad && search[key] != null ? search[key] : searchFields_[tabs][key].value
  
      opts.includes[key] = searchView != BASIC_SEARCH ? searchFields_[tabs][key].options : OPTIONS_DEFAULT
      opts.blanks[key] = searchView != BASIC_SEARCH ? searchFields_[tabs][key].blanks : BLANKS_DEFAULT
    })
    ...
    api.post(`${ENG4900}/search2`,{
      ...
      console.log(data)
      setLoading(false)
      setEng4900s({...eng4900s, [tabs]: data.status != 400 ? data.data : data})
      //setEng4900s()
  ...
  const SearchCriteriaOptions = (val,text="Options",w=200) => {
        ...
        key={`select-${tabs}-opts`}
				value={searchFields_[tabs][val].options ? searchFields_[tabs][val].options : OPTIONS_DEFAULT}
				name={val}
	...
	const SearchBlanksOptions = (val,text="Blanks Options",w=200) => {
        ...
        id={`select-${tabs}-blanks`}
				value={searchFields_[tabs][val].blanks ? searchFields_[tabs][val].blanks : BLANKS_DEFAULT}
				name={val}
      ...
      <div className={tabClasses.root}>
        <AppBar position="static" color="default">
          <Tabs value={tabs} onChange={handleTabChange} aria-label="simple tabs example" textColor="primary" centered indicatorColor="primary"> 
            ...
            <Tab label="Sign Forms" {...a11yProps(1)} />
            {/* <Tab label="Item Three" {...a11yProps(2)} /> */}
          </Tabs>
        </AppBar>
        <TabPanel value={tabs} index={0}>
          {searchForm(0)}
          {Object.keys(eng4900s[0]).length > 0 ? materialTableMyForms(): null}
        </TabPanel>
        <TabPanel value={tabs} index={1}>
          {searchForm(1)}
          {Object.keys(eng4900s[1]).length > 0 ? materialTableSignForms(): null}
        </TabPanel>
        ...
        if(rowData.hasOwnProperty('status')){
          if(rowData.status) {
            if(rowData.hasOwnProperty('tableData')){
              if(rowData.status >= eng4900s[1][rowData.tableData.id].status){
                return true
              }
            }
          }
    ...
    let columns = [
      ...
      lookup: statusOptions, validate: (rowData) => {		
        if(rowData.hasOwnProperty('status')){
          if(rowData.status) {
            if(rowData.hasOwnProperty('tableData')){
              if(rowData.status >= eng4900s[0][rowData.tableData.id].status){
                return true
              }
            }
          }
  ...
  //Render Variables
  const searchTextFieldsGridItems = (tab) => Object.keys(searchFields_[tab]).map(key => {
		const nFields = Object.keys(searchFields_[tab]).length
	...
	<Grid item xs={Math.ceil(12/(nFields + 1))}>                 
      ...
      id={`search-${tab}-${key}`}
      key={`search-${tab}-${key}`} 
			name={key} label={`Search by ${searchFields_[tab][key].label}`} 
			type="search" variant="outlined" 
			value={searchFields_[tab][key].value} 
			onChange={handleSearchFieldsChange}
			onKeyPress={handleSearchKeyPress}
			style={{width:'100%',paddingRight:'20px'}}
			InputLabelProps={{style: {fontSize: '.6vw'}}}
		/>
		{searchFields_[tab][key].value && searchView != BASIC_SEARCH ? <><br/>{SearchCriteriaOptions(key,`${searchFields_[tab][key].label} Options`,w)}</> : null}
		<br/>
		{searchView != BASIC_SEARCH ? SearchBlanksOptions(key,`${searchFields_[tab][key].label} Blanks Options`,w) : null}
	</Grid>
	</>)
  });
	...
	const searchButtonGridItem = (tab) => { 
		return(
			<Grid item style={{textAlign:'left',paddingLeft:'20px'}}  xs={Math.floor(12/(Object.keys(searchFields_[tab]).length))}>
  ...
  return (
    ...
    {create4900.show ? <Eng4900Form action={"CREATE"} type="DIALOG" eng4900s={eng4900s} tab={tabs} setEng4900s={setEng4900s} create4900={create4900} setCreate4900={setCreate4900}/> : null}
    <Header/>
    <div>
      {displayTop()}
