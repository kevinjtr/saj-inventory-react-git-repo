...
import { Snackbar, Box, AppBar, Tabs, Tab, Switch, Typography, TextField,
  ...
  Tooltip,Radio,RadioGroup,Grid, Link, Alert, Autocomplete as AutocompleteV5} from '@mui/material';
import {List as ListIcon, LocationOn as LocationOnIcon, Search as SearchIcon, Computer as ComputerIcon} from '@mui/icons-material';
...
//let sendData = [];
  ...
  const TabsEquipment = React.forwardRef((props, ref) => {
    ...
    const equipment_cols_config = [
        { title: 'HRA Number', field: 'hra_num', type:'numeric', col_id:2.0,width:150,
        editComponent: props => (
        <AutocompleteV5
      ...
      // }
        ...
        {title: 'Status Date', field:'status_date', type:'date', col_id:6.4,editable: 'no' },
        tab_idx === 0 ? {title: 'Update Status', field:'update_status',col_id:6.5 ,editable: 'yes', render: (rowData) => <Link underline="always" component="button" onClick={()=>{console.log(rowData); setSelRowData(rowData); setOpenPopup(true); }}>Update</Link>}: {}
    ] 
