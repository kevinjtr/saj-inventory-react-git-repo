...
const UserDropDown = ({prefersDarkMode, darkModeBackgroundColor, setShowUserDropdown, userName, userLevelName}) => {
	...
	return (
		<ClickAwayListener onClickAway={()=>setShowUserDropdown(false)}>
			<div style={{position:'absolute', width:'200px',right:'10px',top:'35px',border:`1px solid ${prefersDarkMode ? '#404040' : 'rgb(230,230,230)'}`,backgroundColor:prefersDarkMode ? '#404040' : 'rgb(255,255,255)',zIndex:'1200',display:'flex',flexDirection:'column',justifyContent:'space-between'}}>
				<div style={{borderBottom:`1px solid ${prefersDarkMode ? '#404040' : 'rgb(230,230,230)'},padding:'10px',display:'flex',flexDirection:'column`}}>
				...
				<div>
					<button onClick={()=>setOpenProblem(true)} style={{backgroundColor:'rgba(0,0,0,0)',border:'0px',borderTop:'1px solid rgb(230,230,230)',width:'100%',fontSize:'0.75rem',color:'rgb(125,125,125)',padding:'5px',outline:'0px'}}>
						<ErrorIcon style={{fontSize:'0.85rem',color: prefersDarkMode ? '#fff' : 'rgb(125,125,125)',marginBottom:'3px',marginRight:'5px'}}/>
						<a style={{color: prefersDarkMode ? '#fff' : 'rgb(125,125,125)'}}>Submit Feedback</a>
					</button>
				</div>
